---
title: "Frequent Pattern Analysis/Association Rules Mining"
author: "Xuan Pham"
date: "`r Sys.Date()`"
output:
...
What do you think people buy to "stock up" before a hurricane? We would expect that people buy sandbags, flashlights, and other similar items to prepare for hurricanes. Walmart figured out that people buy strawberry pop-tarts too! Here is the original [story](http://www.nytimes.com/2004/11/14/business/yourmoney/what-walmart-knows-about-customers-habits.html). 

Our task for this week is in the similar vein. We will be looking at datasets to see if we can find regularities in them.


# Frequent Pattern Analysis (Association Rules Mining)

We have examined several unsupervised learning models: k-means, k-modes, k-prototype, hierarchical clustering, and principal components analysis. We will examine another unsupervised learning method this week called **frequent pattern analysis (FPA)**.
...
confidence(beer,diaper)=support(beer,diaper)/support(beer) =3/3=1.0
...
**Lift Ratio**: The lift ratio measures the strength of a rule against random chance. The lift ratio overcomes the problem of getting a high confidence value even when the antecedent and consequent itemsets are independent. **We start by assuming that there is no association (i.e. complete independence) between the antecedent and consequent itemsets in a given rule.** We calculate the support for the antecedent itemsets and support for the consequent itemsets separately. We then use the calculated supports to derive a benchmark confidence.  

$Benchmark.Confidence(X->Y)=\frac{support(X)*support(Y)}{support(X)}=support(Y)$  

The benchmark confidence, thus, is nothing more than the support for the consequent itemset. We can expressed the benchmark confidence ratio in terms of relative support for the consequent itemset. n (in the equation below) refers to the number of transactions in a database.  

$Benchmark.Confidencce(X->Y)=\frac{support(Y)}{n}$  

The lift ratio is then calculated as the ratio of confidence to the benchmark confidence.  

$Lift.Ratio=\frac{Confidence(X->Y)}{Benchmark.Confidence(X->Y)}$  
...
library(Matrix)
library(arules)
groceries <- read.transactions("C:/Users/PhamX/Dropbox/Fall_2018/BIA_6301/Week_6/groceries.csv", sep = ",") #9,835 transactions with 169 products.
```
...
```{r}
groceryrules_sorted <-sort(groceryrules, by = c("support", "lift"))
inspect(groceryrules_sorted[1:10])
```
...
```{r}
#plot(groceryrules, engine = 'interactive')
#plot(groceryrules, method="grouped", engine='interactive')
```
...
We will download the dataset into R using a hyperlink. If that does not work, you can import the dataset in as a text file. The text file is posted on Blackboard.  

```{r}
library(Matrix)
library(arules)
#retail <- read.transactions(file="http://fimi.ua.ac.be/data/retail.dat", sep = " ") 
retail <- read.transactions(file="C:/Users/PhamX/Dropbox/Fall_2018/BIA_6301/Week_6/retail.txt", sep = " ") 
summary(retail)
```
