---
title: 'Week 1: Data Mining Process, Exploratory Data Analysis & Multiple Linear Regression'
author: "Xuan Pham"
date: 10/22/2018
output:
...
housing.df <- read.csv("C:/Users/PhamX/Dropbox/Fall_2018/BIA_6301/Week_1/WestRoxbury.csv", header=TRUE) 
```

```{r dimension}
dim(housing.df)  # find the dimension of data frame
head(housing.df, 10)  # show the first six rows
...
corrs2 <- as.matrix(corrs2)
corrplot(corrs2)
corrplot(corrs2, method="number")
```
...
```{r predict} 
pred_step <- predict(stepreg, valid)
library(forecast)
accuracy(pred_step, valid$TOTAL.VALUE)
```

What about the exhaustive regression model? 

```{r predict2}
pred_exhaust <- predict(housing.lm.exhaust, valid)
accuracy(pred_exhaust, valid$TOTAL.VALUE)
```

What about backward selection model?

```{r predict3}
pred_bwd <- predict(bwdreg, valid)
accuracy(pred_bwd, valid$TOTAL.VALUE)
```

What about forward selection model?

```{r predict4}
pred_fwd <- predict(fwdreg, valid)
accuracy(pred_fwd, valid$TOTAL.VALUE)
```
...
| Algorithm            | Excluded Predictor(s)                         | RMSE     |
|----------------------|-----------------------------------------------|----------|
| Exhaustive Search    | BEDROOMS; ROOMS; KITCHEN;YR_BUILT; REMODELOld | 41.73    |
| Backward Elimination | BEDROOMS                                      | 42.58    |
| Forward Selection    | BEDROOMS                                      | 42.58    |
| Stepwise             | BEDROOMS                                      | 42.58    |


Exhaustive search model has the lowest RMSE and the most simplest model too. 

# Step 10: Deploy the model  

It is up to you to decide which model to deploy. The exhaustive search model is an excellent candidate.

### Lift Chart  

The *naive* estimate of a home's value is the mean. The mean value for a home in the validation set is $412,000. We can show the model's mean estimate for each decile (10%) group against the naive estimate. The decile groups are ranked from highest estimated values to lowest estimated values.   

```{r lift}
library(gains)
gain.num <-gains(valid$TOTAL.VALUE, pred_exhaust, groups=10)
